...
WGET=https://gobject.googlecode.com/svn/trunk/owRemote/update.php
...
rm -f $TMP
iwlist $WIFI scanning | grep Address: | cut -d" " -f15 #> $TMP
nvram get wl0_wds | sed -e "s/.\{18\}/&\n/g" > $TMP
while read line
do
  MAC=$line
  QUAL=0
  SIG=`wl rssi $line | cut -d" " -f3`
